...
import type { Cashback } from "@/lib/types/cashback";
...
function dueDaysBorder(days: number): string {
  if (days <= 0) return "border-gray-400 dark:border-gray-600";
  if (days <= 3) return "border-red-400 dark:border-red-600";
  if (days <= 8) return "border-yellow-400 dark:border-yellow-600";
  return "border-green-400 dark:border-green-600";
}

function dueDaysBadge(days: number): string {
  if (days <= 0) return "bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-300";
  if (days <= 3) return "bg-red-100 dark:bg-red-900/30 text-red-700 dark:text-red-300";
  if (days <= 8) return "bg-yellow-100 dark:bg-yellow-900/30 text-yellow-700 dark:text-yellow-300";
            ...
            <button
              ...
              className={`px-4 py-2 text-sm font-medium transition-colors border-b-2 -mb-px cursor-pointer ${
                activeTab === t.key
                  ...
                  : "text-string-muted border-transparent hover:text-string"
              }`}
            >
              {t.label}
            </button>
              ...
              return (
                <div key={card.id} className={`bg-panel rounded-xl border-2 p-4 shadow-sm ${isCredit && debt && !debt.is_paid ? dueDaysBorder(dueIn) : "border-border"}`}>
                  <div className="flex items-start justify-between mb-3">
                    ...
                    {debt?.is_paid && (
                      <span className="px-3 py-1 text-xs font-medium rounded-lg bg-green-100 dark:bg-green-900/30 text-green-700 dark:text-green-300">Pagada</span>
                    )}
                  </div>
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-3 text-sm">
                    <div>
                      <p className="text-xs text-string-muted">{isCredit ? "Límite" : "Saldo"}</p>
                      <p className="font-mono font-medium text-green-600">{card.max_limit != null ? formatCurrency(card.max_limit) : "—"}</p>
                    </div>
                    <div>
                        ...
                        <div className="group relative">
                          ...
                          <div className="absolute left-0 top-full mt-1 w-64 bg-panel border border-border rounded-lg shadow-lg p-3 text-xs z-10 hidden group-hover:block">
                              <div className="space-y-1">
                                <div className="flex justify-between"><span>Compras</span><span className="font-mono text-red-600">-{formatCurrency(calc.total_purchases)}</span></div>
                                <div className="flex justify-between"><span>Plazos (este mes)</span><span className="font-mono text-red-600">-{formatCurrency(calc.total_installments)}</span></div>
                                <div className="flex justify-between"><span>Pagos recurrentes</span><span className="font-mono text-red-600">-{formatCurrency(calc.total_recurring)}</span></div>
            ...
            <>
              <div className="grid grid-cols-2 md:grid-cols-3 gap-4 mb-4">
                <div className="bg-panel rounded-xl border border-border p-4 shadow-sm">
                  ...
                  <p className="text-xl font-bold mt-1 text-indigo-600">{installments.length}</p>
                </div>
                <div className="bg-panel rounded-xl border border-border p-4 shadow-sm">
                  <p className="text-xs text-string-muted uppercase tracking-wider">Restante por pagar</p>
                  <p className="text-xl font-bold mt-1 text-red-600">{formatCurrency(installments.reduce((s, i) => s + Number(i.remaining_months) * Number(i.monthly_amount), 0))}</p>
                </div>
                <div className="bg-panel rounded-xl border border-border p-4 shadow-sm">
                  <p className="text-xs text-string-muted uppercase tracking-wider">Total de cuotas</p>
                  <p className="text-xl font-bold mt-1 text-orange-600">{installments.reduce((s, i) => s + Number(i.total_months), 0)}</p>
                </div>
              </div>
                  ...
                  return (
                    <div key={i.id} className="bg-panel rounded-xl border border-border p-4 shadow-sm">
                      <div className="flex items-start justify-between mb-2">
                        ...
                        <span className={`text-xs px-2 py-0.5 rounded font-medium ${i.remaining_months <= 0 ? "bg-green-100 dark:bg-green-900/30 text-green-700 dark:text-green-300" : i.remaining_months <= 3 ? "bg-yellow-100 dark:bg-yellow-900/30 text-yellow-700 dark:text-yellow-300" : "bg-blue-100 dark:bg-blue-900/30 text-blue-700 dark:text-blue-300"}`}>
                      ...
                      <div className="grid grid-cols-3 gap-3 text-sm">
                        ...
                        <div>
                          ...
                          <p className="font-mono font-medium text-red-600">{formatCurrency(Number(i.monthly_amount))}</p>
                        </div>
                        <div>
                          <p className="text-xs text-string-muted">Restante</p>
                          <p className="font-mono font-medium text-orange-600">{formatCurrency(remainingAmount)}</p>
                        </div>
                      </div>
                    </div>
                  );
      ...
      {activeTab === "eventos" && (
        <div className="space-y-4">
          {!Array.isArray(upcomingEvents) ? (
            <div>
              <p className="text-sm text-red-500">Error: datos de eventos inválidos</p>
            ...
            upcomingEvents.map(ev => (
              <div key={ev.id} className="bg-panel rounded-xl border border-border p-4 shadow-sm flex items-start justify-between">
                ...
                <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${
                  ev.status === "confirmed" ? "bg-green-100 dark:bg-green-900/30 text-green-700 dark:text-green-300" : "bg-yellow-100 dark:bg-yellow-900/30 text-yellow-700 dark:text-yellow-300"
                }`}>
              ...
              {overdueTasks.map(t => (
                <div key={t.id} className="bg-panel rounded-xl border border-red-200 p-3 shadow-sm flex items-center justify-between mb-2">
                  <div>
                    ...
                    <p className="text-xs text-red-500">Vencía el <span className="font-semibold">{new Date(t.due_date!).toLocaleDateString("es-MX")}</span></p>
                  </div>
                  {t.priority > 0 && <span className="text-xs bg-red-100 dark:bg-red-900/30 text-red-700 dark:text-red-300 px-2 py-0.5 rounded-full">Prioridad {t.priority}</span>}
                </div>
              ))}
            </div>
          )}
            ...
            <div>
              ...
              {pendingTasks.map(t => (
                <div key={t.id} className="bg-panel rounded-xl border border-border p-3 shadow-sm flex items-center justify-between mb-2">
                  ...
                  {t.priority > 0 && <span className="text-xs bg-amber-100 dark:bg-amber-900/30 text-amber-700 dark:text-amber-300 px-2 py-0.5 rounded-full">Prioridad {t.priority}</span>}
                </div>
              ))}
            </div>
          )}
                      ...
                      return (
                        <tr key={d.id} className="border-b border-border/50">
                          ...
                          <td className="px-3 py-2 text-center">
                            <span className={`text-xs px-2 py-0.5 rounded-full ${d.is_paid ? "bg-green-100 dark:bg-green-900/30 text-green-700 dark:text-green-300" : "bg-orange-100 dark:bg-orange-900/30 text-orange-700 dark:text-orange-300"}`}>
                    ...
                    {historyService.map(sp => {
                      ...
                      return (
                        <tr key={sp.id} className="border-b border-border/50">
                          ...
                          <td className="px-3 py-2 text-center">
                            <span className={`text-xs px-2 py-0.5 rounded-full ${sp.is_paid ? "bg-green-100 dark:bg-green-900/30 text-green-700 dark:text-green-300" : "bg-orange-100 dark:bg-orange-900/30 text-orange-700 dark:text-orange-300"}`}>
      ...
      {payDialog && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-overlay" onClick={() => setPayDialog(null)}>
          <div className="bg-panel rounded-xl border border-border shadow-xl p-6 w-full max-w-sm mx-4" onClick={e => e.stopPropagation()}>
            ...
            <div className="space-y-3">
              <button onClick={() => handlePayFull(payDialog.debt.id)} className="w-full py-2.5 text-sm font-medium rounded-lg bg-indigo-600 text-white hover:bg-indigo-700 transition-colors">
