    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        {installments.map(i => {
          ...
          return (
            <div key={i.id} className="bg-panel rounded-xl border border-border p-3 shadow-sm">
              <div className="flex items-center justify-between mb-2">
                <span className={`text-[10px] font-medium px-1.5 py-0.5 rounded ${i.remaining_months <= 0 ? "bg-success-bg text-success-text" : i.remaining_months <= 3 ? "bg-warning-bg text-warning-text" : "bg-info-bg text-info-text"}`}>
                  {i.remaining_months}/{i.total_months}
                </span>
              </div>
              <p className="font-semibold text-xs text-string leading-tight mb-1">{i.description}</p>
              <p className="text-[10px] text-string-muted mb-2">{card?.name ?? pm?.name ?? "Sin tarjeta"}</p>
