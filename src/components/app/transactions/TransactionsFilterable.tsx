    ...
    <LocaleProvider locale={locale}>
      <div className="space-y-4">
        ...
        <div className="flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between">
          <div className={FILTER_GRID_CLASS}>
            ...
            <div className={FILTER_WRAP_CLASS}>
              <Select
                ...
                options={[
                  { value: "", label: FILTER_ALL_TYPES(t) },
                  { value: "income", label: t.filter.income },
                  { value: "expense", label: t.filter.expense },
                ]}
                placeholder={FILTER_LABEL_TYPE(t)}
                ariaLabel={FILTER_LABEL_TYPE(t)}
              />
            </div>
            <div className={FILTER_WRAP_CLASS}>
              <Select
          ...
          <button data-create="transactions" className={FILTER_CTA_CLASS}>{t.cta.newTransaction}</button>
        </div>
        ...
        <div className="overflow-x-auto rounded-xl border border-border">
          {loading ? (
            ...
            <div className="p-4 text-center text-string-muted text-sm">{t.empty.transactions}</div>
          ) : (
            <table className="w-full text-sm" aria-label={t.page.transactions}>
              <thead>
                    ...
                    <tr key={tx.id} className="hover:bg-nav-hover border-b border-border/50">
                      ...
                      <td className="px-4 py-3">
                        <span className={`text-xs font-medium px-2 py-0.5 rounded ${tx.type === "income" ? "bg-success-bg text-success-text" : "bg-danger-bg text-danger-text"}`}>
                          {tx.type === "income" ? t.badge.income : t.badge.expense}
                        </span>
                      </td>
                      ...
                      <td className="px-4 py-3 text-right whitespace-nowrap space-x-2">
                        <button data-edit-transactions={tx.id} className="text-primary hover:text-primary-hover text-xs font-medium cursor-pointer">{BTN_EDIT(t)}</button>
                        <button data-delete-transactions={tx.id} className="text-danger hover:text-danger-hover text-xs font-medium cursor-pointer">{BTN_DELETE(t)}</button>
                      </td>
                    </tr>
                  );
                })}
