    ...
    <div className="space-y-4">
      <div className="flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between">
        <div className="grid grid-cols-2 items-end gap-2 sm:flex sm:flex-wrap sm:items-end sm:flex-1">
          <div className={`$"w-full sm:w-48" col-span-2 sm:col-span-1`}>
            <Select
        ...
        <button data-create="pantry" className="hidden sm:block sm:shrink-0 px-3 py-2 text-sm font-medium rounded-lg bg-primary text-white hover:bg-primary-hover cursor-pointer">Nuevo producto</button>
      </div>
                ...
                return (
                  <tr key={i.id} className="hover:bg-nav-hover border-b border-border/50">
                    ...
                    <td className="px-4 py-3 text-right whitespace-nowrap space-x-2">
                      <button data-edit-pantry={i.id} className="text-primary hover:text-primary-hover text-xs font-medium cursor-pointer">{BTN_EDIT_TEXT}</button>
                      <button data-delete-pantry={i.id} className="text-danger hover:text-danger-hover text-xs font-medium cursor-pointer">{BTN_DELETE_TEXT}</button>
                    </td>
                  </tr>
                );
              })}
